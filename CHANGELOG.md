...
- README를 언어별로 분리했다. 지금까지 한국어 하나뿐이라 영어권·중국어권
  사용자는 npm이나 GitHub에서 아무것도 읽을 수 없었다.
  - `README.md` — 영문 랜딩 (언어 선택 · 문서 사이트 · 설치 · 빠른 시작)
  - `README.ko.md` · `README.en.md` · `README.zh-CN.md` — 언어별 전체 문서
- 모든 README 상단에 문서 사이트 링크를 배치했다.

> **0.3.0은 npm에 배포되지 않았다.** git 태그로만 남아 있다.
> 0.2.2에서 올라오는 경우 아래 0.3.0 항목의 변경 사항이 모두 함께 적용된다.
